...
class Plotter(object):
    ...
    def _make_layout(self, signals):
        ...
        if signals:
            ...
            # plot-in-plot
            elif isinstance(signals, list) and isinstance(signals[0], Signal):
                signals = [signals]
                self._layout = self._LAYOUT_PIP
            # grid plot
            elif isinstance(signals, list) and isinstance(signals[0], list):
                # check whether it's a valid grid layout
                # (all rows with equal size and not empty)
                for row in signals:
                    if len(signals[0]) == 0 or len(row) != len(signals[0]):
    ...
    def graph(self):
        ...
        for i in range(nrows):
            for j in range(ncols):
                ...
                if not self.zrange or _utl.all_v(0, self.zrange):
